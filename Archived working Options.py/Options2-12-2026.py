...
# COMMENTED OUT - Using year-specific CSV files instead
        ...
        try:
            ...
            with open(self.file_name , mode='r') as file:  
                ...
                # Prepare the totals row, aligning with the correct columns
                    ...
                    print("\n")
                    #Save to file and print to console to preserve formatting with tabulate and avoid issues with print limitations on large tables in some environments

                    with open("output_file", mode='w') as file:
                        file.write(tabulate.tabulate(rows, headers=headers_with_row, tablefmt='PIPE'))
        ...
        with open(self.file_name , mode='r') as file:
              ...
              if len(data) > 1:
                ...
                if filtered_data == []:
                   ...
                   with open("output_file", mode='w') as file:
                        file.write(tabulate.tabulate(filtered_rows, headers=headers_with_row, tablefmt='PIPE'))
                        with open("output_file", mode='r') as file:
                            for line in file:
                                print(line, end="")
                            print(file.read())
...
def main():
    while True:
        ...
        #debugging line
         ...
         #print(f"Current working directory: {os.getcwd()}")  #debugging line
             file_name = f"Options_{datetime.datetime.now().year}.csv"
