    ...
    # Amend trade records
      ...
      if Amend_delete == "A":
        ...
        # Loop through each field and ask for new values
           ...
           #zip_result =zip(headers, rows[trade_row-1]) #Skip first element - row; debugging line
            #debugging line
            #print(f"Debug: Zip result: ", list(zip_result))  #debugging line
            for field, validation_method in field_mapping.items():     #(zip(headers, rows[trade_row-1])):   
                
                    # Edit all other fields
                if not field == "Profit/Loss":  # Skip Profit/Loss field
