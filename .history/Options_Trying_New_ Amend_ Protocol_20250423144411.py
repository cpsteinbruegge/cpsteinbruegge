...
class Options:
    ...
    def choose_option(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                self.Options_Trade_Editor(self.file_name, Amend_delete)  
                # os.remove(self.file_name) 
            elif choice == "5":
                Amend_delete = "D" #Delete trade records
                self.Options_Trade_Editor(self.file_name, Amend_delete) #Amend trade records  
                # os.remove(self.file_name) 
            elif choice == "6":
                sys.exit(0)
            else:
    ...
    def view_trades(self,row_number=None):
        ...
        try:
            ...
            with open(self.file_name , mode='r') as file:  
                ...
                #   print(f"Debug: Data read from file: {data}")   #debugging line
                ...
                if len(data) > 1:
                    ...
                    if row_number is not None:
                        if 1 <= row_number <= len(rows):
                            ...
                            print(tabulate.tabulate([[row_number] + rows[row_number-1]], headers=["Row"]+headers,tablefmt='PIPE'))  
                            return rows[row_number-1] # Return the specific row as a list
                        ...
                        return rows   # Return all rows with data as a list, excluding the header row, starting from index 1; this list will start from 0.
    ...
    def Options_Trade_Editor (self,file_name, Amend_delete):
    ...
    # Amend trade records
      ...
      if Amend_delete == "A":
        ...
        if 1 <= trade_row <= len(rows): 
            ...
            print("Reached field mapping")  #debugging line
            ...
            "Total Open Premium": Get_valid_input.get_valid_float,  
            ...
            print(f"Field mapping complete: {field_mapping}")      
        ...
        # Loop through each field and ask for new values
            ...
            zip_result = list(zip(headers, rows[trade_row-1])) #Skip first element - row; debugging line
            #debugging line
            print(f"Debug: Zip result: ", list(zip_result))  #debugging line
            zip_result_list = list(zip_result)  # Convert to a list to reuse it

            for (header, value) in zip_result_list:     #(zip(headers, rows[trade_row-1])):   
                ...
                if not header == "Profit/Loss":  # Skip Profit/Loss field
                    ...
                    while True: #Check for space bar or enter key
                      try:
                           ...
                           if user_input == "": 
                              ...
                              break # If Enter is pressed, keep the original value
                              
                                                       
                           
                      except Exception as e:
                           print("Invalid Key; please press <Space Bar> or <Enter> to continue.")
                               
                    if user_input == "":
                        # If Enter is pressed, keep the original value
                        print(f"Keeping original value for {header}: {value}", end="")  
                        updated_row.append(value)
                        print()
 
                    elif user_input == " ":
                        # If Space Bar is pressed, ask for a new value  

                         if validation_method:
                    # Validate the new value using the corresponding method
                                  try:
                                     validated_value = validation_method(f"Enter new value for {header}: ")
                            ...
                            # If no validation method is found, keep the original value
                            updated_row.append(value)
                            print(f"Debug - No validation method for {header}. Keeping original value: {value}")    
                    ...
                    try:
                        ...
                        profit_loss = int(total_open_premium - close_cost)
                        updated_row.append(f"{profit_loss}")  # Append recalculated "Profit/Loss"
           ...
           # Update the row in the list
                    ...
                    print()
                    #print(f"Updated trade details after amendment: {rows[trade_row - 1]}",end="")  #debugging line
                    print()
